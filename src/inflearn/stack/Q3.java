...
public class Q3 {
    ...
    public int solution(int n, int m, int[][]board, int[] moves) {
        ...
        for(int i=0; i<m; i++){
            int point = moves[i];
            int  j=0;
            while(j<n && board[point][j] == 0){
                j++;
            }

            if(j!=n) {
                board[point][j] = 0;
                if(stack.isEmpty() || board[point][j] != stack.peek()){
                    stack.push(board[point][j]);
                } else {
                    stack.pop();
                    answer += 2;
                    break;
...
//                board[j][i] = in.nextInt();
        ...
        Q3 q = new Q3();
        int result = q.solution(n, m, board, moves);
